"""
Generate witness data for fixed-point addition verification.
"""
import sys
import toml
import mpmath
...
def generate_fp_add_witness(num_reps, log_scale):
    """Generate witness data for fixed-point addition checks."""
...
def write_constants(num_reps, log_scale):
    ...
    constants = f"""// Number of addition checks to perform
...
"""
    ...
    with open('src/fp_add/constants.nr', 'w') as f:
        f.write(constants)

def write_prover_toml(witnesses):
    """Write Prover.toml file."""
    prover_data = {'fp_add_wits': witnesses}
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python fp_add_gen.py <num_reps> <log_scale>")
        print("Example: python fp_add_gen.py 100 120")
        sys.exit(1)
    
    num_reps = int(sys.argv[1])
    log_scale = int(sys.argv[2])
    
    print(f"Generating {num_reps} fixed-point addition witnesses with log_scale={log_scale}")
    ...
    # Generate witnesses
    witnesses = generate_fp_add_witness(num_reps, log_scale)
